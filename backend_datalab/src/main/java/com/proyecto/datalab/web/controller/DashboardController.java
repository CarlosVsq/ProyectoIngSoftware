    ...
    private Map<String, Object> calcularSexo() {
        ...
        respuestaRepository.findAll().stream()
            ...
            .forEach(r -> {
                ...
                if (val.contains("masc")) conteo.get("masculino").incrementAndGet();
                else if (val.contains("fem")) conteo.get("femenino").incrementAndGet();
                else conteo.get("otros").incrementAndGet();
            });
        ...
        respuestaRepository.findAll().stream()
            ...
            .forEach(r -> {
                try {
                    ...
                    if (edad >= 18 && edad <= 30) conteo.get("18-30").incrementAndGet();
                    else if (edad <= 45) conteo.get("31-45").incrementAndGet();
                    else if (edad <= 60) conteo.get("46-60").incrementAndGet();
                    else if (edad <= 75) conteo.get("61-75").incrementAndGet();
                    else conteo.get("76+").incrementAndGet();
